...
class OfferController extends Controller
{
    ...
    public function filterChallengeOffers(Request $request)
    {
        ...
        $sum = 0;
        if($option === 'Cena-max'){
            $offers = $challenge->offers()->orderBy('price_of_delivery', 'DESC')->where('rejected', '<>', 1)->with('solution')->get();
        }else if($option === 'Cena-min'){
            $offers = $challenge->offers()->orderBy('price_of_delivery', 'ASC')->where('rejected', '<>', 1)->with('solution')->get();
