<template>
    <div class="w-full flex items-center mt-3">
        <div class="w-8 h-8 flex-none image-fit mr-3" v-if="comment.commentator.id != user.id">
            <Avatar :src="'/s3/avatars/' + comment.commentator.avatar" :username="comment.commentator.name + ' ' + comment.commentator.lastname" size="40" color="#FFF" background-color="#930f68"/>
        </div>
        ...
        <div class="w-8 h-8 flex-none image-fit ml-3" v-if="comment.commentator.id == user.id">
            <Avatar :src="'/s3/avatars/' + user.avatar" :username="user.name + ' ' + user.lastname" size="40" color="#FFF" background-color="#930f68"/>
            </div>
    </div>
</template>
