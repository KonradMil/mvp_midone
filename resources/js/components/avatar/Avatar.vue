<template>
    <div class="vue-avatar--wrapper" :style="[style, customStyle]" aria-hidden="true">
        ...
        <span v-show="!isImage" style="font-weight: 900;">{{ userInitial }}</span>
    </div>
</template>
    ...
    setup(props, { emit }) {
       ...
       const  lightenColor =  (hex, amt) => {
        ...
        const isImage = computed(() => {
            return !imgError.value && Boolean(props.src) && props.src != 'uploads/null' && props.src != 'uploads/undefined'
        })
        ...
        const userInitial = computed(() => {
            if (!isImage.value) {
                return props.initials || getInitials(props.username)
            }
            return ''
        })
