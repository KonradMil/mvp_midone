    ...
    setup(props) {
        ...
        const finalPartsList = computed(() => {
            if(challenge.value.save_json != undefined) {
                ...
                return JSON.parse(props.solution.save_json).parts.filter(part => JSON.parse(challenge.value.save_json).parts.forEach(part2 => !part2.unity_id.includes(part.unity_id)));
            }
        });
